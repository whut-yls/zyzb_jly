...
static void low_level_init(struct netif *netif)
{
  ...
  /* Start ETH HAL Init */
   ...
   uint8_t MACAddr[6] ;
  heth.Instance = ETH;
  MACAddr[0] = gDeviceParam.rj45Arg.MacAddress[0];
  MACAddr[1] = gDeviceParam.rj45Arg.MacAddress[1];
  MACAddr[2] = gDeviceParam.rj45Arg.MacAddress[2];
  MACAddr[3] = gDeviceParam.rj45Arg.MacAddress[3];
  MACAddr[4] = gDeviceParam.rj45Arg.MacAddress[4];
  MACAddr[5] = gDeviceParam.rj45Arg.MacAddress[5];
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  TxConfig.TxBuffer = Txbuffer;
	...
	#define	User_Send_IT_Clean_DCache	//add
	SCB_InvalidateDCache();		//add
	HAL_ETH_Transmit_IT(&heth,&TxConfig);
//  HAL_ETH_Transmit(&heth, &TxConfig, ETH_DMA_TRANSMIT_TIMEOUT);	//del
...
//#if defined(DUAL_CORE) || defined(CORE_CM7)
    /* Invalidate data cache for ETH Rx Buffers */
		#define	User_Enable_InvalidateDCache_low_level_input
...
//#if !defined(DUAL_CORE) || defined(CORE_CM7)
  /* Invalidate data cache: lwIP and/or application may have written into buffer */
	#define	User_Enable_InvalidateDcache_pbuf_free_custom
...
unsigned portBASE_TYPE ethernet_link_thread_stack;
/**
...
void ethernet_link_thread(void const * argument)
{
  uint32_t PHYLinkState;
	...
	while(1)
	{
//		ethernet_link_thread_stack=uxTaskGetStackHighWaterMark(ethernetTaskTaskHandle);
		PHYLinkState = LAN8742_GetLinkState(&LAN8742);
		if(PHYLinkState <= LAN8742_STATUS_LINK_DOWN)
		{
			gEthStatus=false;
			if(isNetState){ 																									//原来时连接正常 突然连接失败了 断开
					gGlobalData.rj45Status=false;
					isNetState=false;
				 
					send_NetSync(0);    																				//网络异常灯 1601  //**//
					gGlobalData.isSendMq=false;																	//强制再发送一次MQ信息
					gGlobalData.isConnect=0;																		//断网
					gGlobalData.conFlage=2;																			//切wifi
					HAL_GPIO_WritePin(GPIOC,GPIO_PIN_12,GPIO_PIN_SET);   		    //网络灯绿色不亮
			...
			if(!isNetState)
				{
					...
					gGlobalData.conFlage=1;																			//网络标志位最重要标志，不能给网口一直切
					gGlobalData.rj45Status=true;
...
//hgz    HAL_ETH_Stop_IT(&heth);
  ...
  else if(!netif_is_link_up(netif) && (PHYLinkState > LAN8742_STATUS_LINK_DOWN))
  {
	...
	#define	Bug_In_ethernet_link_thread
//hgz	HAL_ETH_Start_IT(&heth);
  netif_set_up(netif);
  netif_set_link_up(netif);
  }
//  HAL_ETH_Start_IT(&heth);
