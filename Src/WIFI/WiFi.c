...
bool _wifi_AtMqtt_conn = false,_wifi_AtMqtt_sub = false,Echo_Wifi_flag = true;
...
//	strcpy((char*)gDeviceParam.wifiArg.WifiPwd,"123456ming");
	...
	while(1)
	{
		...
		/*数据接受解析处理*/  	
			if((cjdata = strstr((const char*)RecCom3,MQTT_DATA_REC))!=0 ){  																																													//心跳包不进解析		
...
{
	sprintf(rec_cmd,"%s\r\n",cmd);	//发送命令
	HAL_UART_Transmit_Wifi((uint8_t *)rec_cmd,strlen(rec_cmd),0xffff);
	return;
} 
//wifi初始化
void wifi_init(void)
{
	...
	osDelay(200);
	if(Echo_Wifi_flag == true){
		Echo_Wifi_flag = false;
		atk_8266_send_cmd("ATE0",0xFFFF);                         //TRUN OFF WIFI ECHO
		osDelay(200);	
	}
	return;
}
